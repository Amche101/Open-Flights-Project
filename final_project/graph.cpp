...
const Node Graph::InvalidNode = Node("_CS225INVALIDNODE",-1.0,-1.0);
...
Node Graph::getNode(Vertex v) const 
{
...
void Graph::insertNode(Vertex v, Node n)
{
    // will overwrite if old stuff was there
    removeNode(v);
    // make it empty again
...
Node Graph::removeNode(Vertex v) {
    if (vertex_list.find(v) != vertex_list.end()) {
        Node ret = vertex_list[v];
        vertex_list.erase(v);
        return ret;
    }
    return InvalidNode;
...
void Graph::print() const
{
    for (auto it = adjacency_list.begin(); it != adjacency_list.end(); ++it) 
    {
        cout << vertex_list[it->first].name_<<endl;
        cout << "ID: "<<it->first << endl;
        cout << "Latitude: "<<vertex_list[it->first].latitude_<<endl;
        cout << "Longitude: "<<vertex_list[it->first].longitude_<<endl;

        for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) 
        {
            std::stringstream ss;
            cout <<"    => "<< vertex_list[it2->first].name_<<endl;
            ss << it2->first; 
...
Node Graph::setNodeLabel(Vertex v, string label) {
    if (assertVertexExists(v, "") == false)
    return InvalidNode;
    vertex_list[v].label_ = label;
    return vertex_list[v];
}
...
string Graph::getNodeLabel(Vertex v) const {
    if (assertVertexExists(v, "") == false)
    return InvalidLabel;
    return vertex_list[v].label_;
}

void Graph::BFS () 
{
    for(Vertex v : getVertices()) {
        vertex_list[v].label_= "UNEXPLORED";
    }
    for (Edge e : getEdges()) {
        setEdgeLabel(e.source,e.dest,"UNEXPLORED");
    }
    for(Vertex v: getVertices()) {
        if (vertex_list[v].label_ == "UNEXPLORED")
        BFS(v);
    }
}
...
{
    queue<Vertex> q;
    vertex_list[v].label_ = "VISITED";
    q.push(v);
    while (!q.empty()) {
        v = q.front();
        q.pop();
        for (Vertex w : getAdjacent(v)) {
            if (vertex_list[w].label_ == "UNEXPLORED") {
                setEdgeLabel(v,w,"DISCOVERY");
                vertex_list[w].label_  = "VISITED";
                q.push(w);
