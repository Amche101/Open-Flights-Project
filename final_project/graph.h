 ...
 *
 * Written for CS 225 Spring 2020
 * @author Haijian Wang
 *
 */
#pragma once
...
public:
     ...
     * @param v - the vertex we want to get node for 
     * @return an information node for the vertex
     */
    Node getNode(Vertex v) const;
     ...
     *         - if edge doesn't exist, return InvalidEdge
     */
    Node setNodeLabel(Vertex v, string label);
     ...
     *         - if edge doesn't exist, return InvalidLabel
     */
    string getNodeLabel(Vertex v) const;
     ...
     * @param v - the name for the vertex, creating an empty node for vertex_list
     */
    void insertVertex(Vertex v);

    /**
     * Inserts a new node into the graph and initializes its label as "".
     * @param v - the name for the vertex
     * @param n - the information node associated with the vertex
     */
    void insertNode(Vertex v, Node n);
    ...
    /**
     * Removes a given node from the vertex_list.
     * @param v - the vertex to remove
     * @return - if v exists, return v
     *         - if not, return InvalidNode;
     */
    Node removeNode(Vertex v);
    ...
    /**
     * run BFS traversal on the Graph and label the Graph
     * for directed graph, we only care about the edges going out 
     * of a vertex
     * Notice: this will reset all the labels first then run the BFS
     */
    void BFS();
    
    /**
     * run BFS traversal on the Graph from a given vertex
     * @param v - the starting vertex for traversal
     * Notice: all the edges and nodes are assumed to be properly labeled
     */
    void BFS(Vertex v);
    ...
    const static Node InvalidNode;

private:
    mutable unordered_map<Vertex, unordered_map<Vertex, Edge>> adjacency_list;
    mutable unordered_map<Vertex, Node> vertex_list;
